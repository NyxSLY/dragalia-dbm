...
def write_timeline(input_file):
    ...
    # file head
    out.write(r'import {ITimelineMore} from "@/assets/timeline/type";'+'\n')

    out.write(f'export const {name}: Record<string, ITimelineMore> = {{}};\n\n\n')
    ...
    for line in x:
        ...
        outline = f'{name}.{i} = {{ name: \"{skill_name}\", time: {{minute: {str(minute)}, second: {str(second)}}} }};\n'
        out.write(outline)
        if i[-1] == 'z':
            i = len(i)*'a' + 'a'
        else:
